    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-gray-500 font-medium">Loading inventory profiles...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-xl border border-gray-200/60 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="font-mono text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded border border-gray-200/40">
                  {product.sku}
                </span>
              </div>
              <p className="text-[11px] text-gray-400 mt-0.5">Slug: <span className="font-mono">{product.slug}</span></p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {product.is_featured && (
              <span className="px-2.5 py-1 rounded-full font-bold tracking-wide text-[10px] uppercase bg-yellow-50 border border-yellow-200/60 text-yellow-700">
                Featured Cover
              </span>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="md:col-span-5 space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200/60 shadow-xs space-y-3">
              <div className="aspect-square bg-gray-50 border border-gray-100 rounded-lg overflow-hidden flex items-center justify-center relative">
                {activeImage ? (
                  <img src={activeImage} alt={product.name} className="object-contain max-h-full w-full p-2" />
                ) : (
                  <div className="text-gray-400 font-medium">No Image Configured</div>
                )}
              </div>
                  ...
                  {product.images.map((img, i) => (
                    <button
                      ...
                      className={`aspect-square rounded border transition-all overflow-hidden p-1 bg-white ${
                        activeImage === img.image_url ? 'border-gray-950 ring-2 ring-gray-950/10' : 'border-gray-200 hover:border-gray-400'
                      }`}
                    >
            ...
            <div className="bg-white rounded-xl border border-gray-200/60 shadow-xs overflow-hidden">
              ...
              <div className="p-4 divide-y divide-gray-100 max-h-[380px] overflow-y-auto">
                {product.reviews && product.reviews.length > 0 ? (
                  product.reviews.map((review) => (
                    <div key={review.id} className="py-3 first:pt-0 last:pb-0 space-y-2">
                          ...
                          <div className="flex items-center gap-0.5">
                            {[...Array(5)].map((_, index) => (
                              <Star 
                                key={index} 
                                size={11} 
                                className={index < review.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'} 
                              />
                            ))}
                            <span className="ml-1 text-[11px] font-bold text-gray-800">{review.rating}/5</span>
                          </div>
                          <p className="text-[10px] text-gray-400 font-mono tracking-tight">UID: {review.user_id}</p>
                        </div>
                      ...
                      <div className="p-2 bg-gray-50 border border-gray-100 rounded-md text-gray-600 font-sans italic text-[11px]">
                        {review.comment ? (
                          `"${review.comment}"`
                        ) : (
                          <span className="text-gray-400 not-italic text-[10px]">No comment left with rating score.</span>
                        )}
                      </div>
                  ...
                  <div className="text-center py-6 text-gray-400 italic">No feedback submissions found on this inventory profile.</div>
                )}
              </div>
            </div>
          ...
          <div className="md:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white p-3.5 rounded-xl border border-gray-200/60 shadow-xs">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Base Price</p>
                <p className="text-lg font-bold text-gray-900 mt-1 font-mono">₵{product.price?.toFixed(2)}</p>
              </div>
              <div className="bg-white p-3.5 rounded-xl border border-gray-200/60 shadow-xs">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Sale Price</p>
                <p className="text-lg font-bold text-yellow-600 mt-1 font-mono">
                  {product.sale_price > 0 ? `₵${product.sale_price.toFixed(2)}` : '—'}
                </p>
              </div>
              <div className="bg-white p-3.5 rounded-xl border border-gray-200/60 shadow-xs">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Stock Available</p>
                <div className="flex items-center gap-1.5 mt-1">
                  <Package size={14} className={product.stock_quantity > 0 ? 'text-gray-700' : 'text-rose-500'} />
                  <span className={`text-lg font-bold font-mono ${product.stock_quantity === 0 ? 'text-rose-600' : 'text-gray-900'}`}>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200/60 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <span className="block text-gray-400 font-medium mb-1 flex items-center gap-1"><Tag size={11}/>Brand Entity</span>
                  <div className="px-2.5 py-1.5 bg-gray-50 border border-gray-200/40 rounded-lg text-gray-800 font-semibold">
                    {product.brand?.name || product.brand_id || 'Direct / Generic'}
                  </div>
                </div>

                <div>
                  <span className="block text-gray-400 font-medium mb-1 flex items-center gap-1"><Globe size={11}/>Wine Regions</span>
                  <div className="flex flex-wrap gap-1">
                    {product.wine_regions?.map((r) => (
                      <span key={r.id} className="bg-gray-100 border border-gray-200 text-gray-800 px-2 py-0.5 rounded font-medium">{r.name}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-4 rounded-xl border border-gray-200/60 shadow-xs space-y-3">
                ...
                {product.wine_attributes && product.wine_attributes.length > 0 ? (
                  ...
                  <p className="text-gray-400 italic py-2">No structural properties defined.</p>
                )}
              </div>
                ...
                {product.pairings && product.pairings.length > 0 ? (
                  <div className="space-y-2 max-h-[160px] overflow-y-auto">
                    {product.pairings.map((pair, i) => (
                      <div key={i} className="p-2 bg-gray-50 border border-gray-200/40 rounded-lg">
                        <span className="block font-bold text-gray-950">{pair.dish?.name || `Dish ID: ${pair.dish_id}`}</span>
                        {pair.reason && <p className="text-[11px] text-gray-500 mt-0.5 leading-normal">"{pair.reason}"</p>}
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-gray-400 italic py-2">No menu items aligned to this product.</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200/60 shadow-xs">
              <h3 className="font-bold text-gray-400 text-[10px] uppercase tracking-wider border-b pb-1.5 mb-3">Freight & Dimensional Metrics</h3>
              <div className="grid grid-cols-4 gap-2 text-center font-mono text-[11px]">
                <div className="bg-gray-50 p-2 border rounded-md"><span className="block text-[10px] text-gray-400 font-sans font-medium mb-0.5">Weight</span>{product.weight || 0} kg</div>
                <div className="bg-gray-50 p-2 border rounded-md"><span className="block text-[10px] text-gray-400 font-sans font-medium mb-0.5">Length</span>{product.length || 0} cm</div>
                <div className="bg-gray-50 p-2 border rounded-md"><span className="block text-[10px] text-gray-400 font-sans font-medium mb-0.5">Width</span>{product.width || 0} cm</div>
                <div className="bg-gray-50 p-2 border rounded-md"><span className="block text-[10px] text-gray-400 font-sans font-medium mb-0.5">Height</span>{product.height || 0} cm</div>
              </div>
            </div>
