      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            Configure step-tier weight structures, coordinate cost tables, and
            define global territorial parameters.
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <select
            value={zoneIdFilter}
            onChange={(e) => {
              setZoneIdFilter(e.target.value);
              setCurrentPage(1);
            }}
        ...
        {loading && shippingRates.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <span className="text-xs text-zinc-400">
              Loading active weight class frameworks...
            </span>
          </div>
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xs font-bold text-zinc-700">
              No freight rules currently verified in database
            </h3>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-zinc-600">
                <thead className="bg-zinc-50 text-[10px] font-bold text-zinc-400 uppercase tracking-wider border-b border-zinc-100">
                  <tr>
                    <th
                      className="py-3 px-5 cursor-pointer hover:bg-zinc-100 transition-colors"
                      onClick={() => handleSort("shipping_zone_id")}
                    >
                      <div className="flex items-center gap-1.5">
                        Target Logistics Zone Identifier{" "}
                        <ArrowUpDown size={10} />
                      </div>
                    </th>
                    <th
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        Minimum Weight <ArrowUpDown size={10} />
                      </div>
                    </th>
                    <th
                      className="py-3 px-5 cursor-pointer hover:bg-zinc-100 transition-colors"
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        Maximum Weight <ArrowUpDown size={10} />
                      </div>
                    </th>
                    <th
                      className="py-3 px-5 cursor-pointer hover:bg-zinc-100 transition-colors"
                  ...
                  {shippingRates.map((rate) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="font-bold font-mono text-zinc-950 tracking-wide uppercase">
                              {availableZones.find(
                                (z) => z.id === rate.shipping_zone_id,
                              )?.name || "Unlinked Custom Region"}
                            </div>
                      ...
                      <td className="py-3.5 px-5 font-mono text-zinc-900 text-sm">
                        {Number(rate.min_weight).toFixed(2)}{" "}
                        <span className="text-[10px] text-zinc-400 font-sans font-normal">
                          kg
                        </span>
                      </td>
                      <td className="py-3.5 px-5 font-mono text-zinc-900 text-sm">
                        {Number(rate.max_weight).toFixed(2)}{" "}
                        <span className="text-[10px] text-zinc-400 font-sans font-normal">
                          kg
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white border border-zinc-200 shadow-2xl rounded-xl overflow-hidden z-10 animate-in fade-in zoom-in-95 duration-150">
            <div className="p-5 border-b border-zinc-100 bg-zinc-50 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-zinc-900">
                  {selectedRate
                    ? "Modify Freight Scale Metrics"
                    : "Incorporate Freight Weight Matrix Step"}
                </h3>
                <p className="text-[11px] text-zinc-500 mt-0.5">
                  Configure strict mass ranges mapping to structural costs.
                </p>
              </div>
              <button
                onClick={() => setIsFormModalOpen(false)}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <div className="relative flex items-center">
                    ...
                    />
                    <span className="absolute right-3 font-medium text-zinc-400">
                      kg
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <label className="block font-semibold text-zinc-700">
                    Ceiling Mass Target (max_weight) *
                  </label>
                  <div className="relative flex items-center">
                    <input
                      type="number"
              ...
              <div className="space-y-1">
                <label className="block font-semibold text-zinc-700">
                  Assigned Logistics Rate Price Matrix (₵) *
                </label>
                <div className="relative flex items-center">
                  <p className="absolute left-3 text-zinc-400" size={14}>
                    ₵
                  </p>
                  <input
                    type="number"
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-zinc-900">
                  Purge Selected Freight Weight Class?
                </h4>
                <p className="text-zinc-500 leading-relaxed">
                  Are you certain you want to destroy the logistics threshold
                  running from **{selectedRate?.min_weight}kg** to **
                  {selectedRate?.max_weight}kg** under zone mapping **
                  {selectedRate?.shipping_zone_id}**? Dynamic shopping cart rate
                  requests matching this segment will default to general zone
                  metrics instantly.
                </p>
              </div>
            </div>
