...
const Blogs = () => {
  ...
  const { posts, loading, mutationLoading, error, message } = useSelector(
    (state) => state.blogs,
  );
  ...
  const openEditDrawer = (post) => {
    ...
    setVideoSource(
      typeof post.video_url === "string" && post.video_url ? "url" : "upload",
    );
    setFormData({
      title: post.title || "",
      slug: post.slug || "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            Draft stories, manage analytical parameters, and link asset
            attachments.
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-xl border border-zinc-200/80 shadow-sm flex items-center gap-3">
        ...
        <input
          type="text"
          placeholder="Search blog titles or paths..."
                  ...
                  >
                    <td className="py-4 px-5">
                      <div className="font-semibold text-zinc-900 flex items-center gap-2">
                        {post.title}
                        {post.is_featured && (
                          <span className="bg-amber-50 border border-amber-200 text-amber-800 font-bold text-[9px] tracking-wide px-1.5 py-0.2 rounded">
                            Featured
                          </span>
                        )}
                      </div>
                    ...
                    <td className="py-4 px-5 capitalize font-mono text-zinc-500">
                      {post.type}
                    </td>
                    <td className="py-4 px-5">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold border ${post.is_published ? "bg-emerald-50 border-emerald-100 text-emerald-800" : "bg-zinc-50 border-zinc-200 text-zinc-500"}`}
                      >
                    ...
                    <td className="py-4 px-5 text-right space-x-2">
                      ...
                      <button
                        ...
                        className="inline-flex p-1.5 rounded-md border border-red-100 bg-white text-red-600 hover:bg-red-50"
                      >
                        <Trash2 size={13} />
                      </button>
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-full max-w-lg h-screen bg-white shadow-2xl flex flex-col justify-between z-50 animate-slide-in">
              <div className="p-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-50">
                <div>
                  <h3 className="font-serif font-bold text-zinc-900 text-sm">
                    {editingPost ? "Edit Story Specs" : "Draft Core Content"}
                  </h3>
                  <p className="text-[10px] text-zinc-400 mt-0.5">
                    Inject metrics, binary attachments, and metadata scopes.
                  </p>
                </div>
                <button
                  onClick={closeDrawer}
              ...
              <form
                ...
                className="flex-1 overflow-y-auto p-5 space-y-5 text-xs pb-28 custom-scrollbar"
              >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={formData.type}
                        onChange={handleInputChange}
                        className="w-full px-3 py-2 border border-zinc-200 rounded-lg bg-white capitalize"
                      >
                  ...
                  <div>
                    ...
                    {imageSource === "url" ? (
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 border border-zinc-200 rounded-lg file:mr-3 file:py-1 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-bold file:bg-zinc-100 file:text-zinc-700 cursor-pointer"
                      />
                    )}
                  </div>
                    ...
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex gap-1 bg-zinc-100 p-0.5 rounded-md border border-zinc-200">
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 ${videoSource === "url" ? "bg-white text-zinc-900 shadow-xs" : "text-zinc-500"}`}
                        >
                          <Link size={10} />
                          URL
                        </button>
                        <button
                          ...
                          className={`px-2 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 ${videoSource === "upload" ? "bg-white text-zinc-900 shadow-xs" : "text-zinc-500"}`}
                        >
                          <Upload size={10} />
                          Upload
                        </button>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className="w-full px-3 py-2 border border-zinc-200 rounded-lg"
                        placeholder="wine, harvest"
                      />
                    </div>
                    <div>
                      <label className="block font-semibold text-zinc-700 mb-1">
                        Publication Timestamp Calendar
                      </label>
                      <input
                        type="datetime-local"
                        name="published_at"
                        ...
                        onChange={handleInputChange}
                        className="w-full px-3 py-2 border border-zinc-200 rounded-lg font-mono text-zinc-800"
                      />
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#c4945c]">
                    SERP SEO Architecture Tuning
                  </span>
                  <div>
                    <label className="block font-semibold text-zinc-700 mb-1">
                      Meta Customize Title
                    </label>
                    <input
                      type="text"
                      name="meta_title"
      ...
      <ConfirmDeleteModal
        ...
        message={`Are you entirely sure you want to delete "${itemToDelete?.title}"? This will drop all structural metrics recorded for this story node across the public cache indexes.`}
      />
    </div>
  );
};
