...
export const fetchBlogs = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(err.response?.data?.message || 'Failed to fetch blog posts');
    }
  }
);
...
export const createBlog = createAsyncThunk(
  ...
  async (blogData, { rejectWithValue }) => {
    try {
      ...
      const hasFile = blogData.featured_image instanceof File || blogData.video_url instanceof File;
...
const blogSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createBlog.pending, (state) => { state.mutationLoading = true; state.error = null; })
      .addCase(createBlog.fulfilled, (state, action) => {
        state.mutationLoading = false;
        state.posts.unshift(action.payload);
        state.message = "Blog post created successfully";
      })
      .addCase(createBlog.rejected, (state, action) => { state.mutationLoading = false; state.error = action.payload; })

      .addCase(updateBlog.pending, (state) => { state.mutationLoading = true; state.error = null; })
      .addCase(updateBlog.fulfilled, (state, action) => {
        state.mutationLoading = false;
        state.posts = state.posts.map((post) => post.id === action.payload.id ? action.payload : post);
        state.message = "Blog post updated successfully";
      })
      .addCase(updateBlog.rejected, (state, action) => { state.mutationLoading = false; state.error = action.payload; })

      .addCase(deleteBlog.pending, (state) => { state.mutationLoading = true; state.error = null; })
      .addCase(deleteBlog.fulfilled, (state, action) => {
        state.mutationLoading = false;
        state.posts = state.posts.filter((post) => post.id !== action.payload);
        state.message = "Blog post deleted successfully";
      })
      .addCase(deleteBlog.rejected, (state, action) => { state.mutationLoading = false; state.error = action.payload; });
  },
});
